...
def main(neural_network, parameter):
    ...
    while True:
        ...
        crashInfo = mainGame(movementInfo, neural_network, parameter)
...
def mainGame(movementInfo, neural_network, parameter):
    ...
    if parameter is 0:
	    ...
	    newPipe1 = getshiftedUpPipe()
	    newPipe2 = getshiftedUpPipe()
    elif parameter is 3:
        newPipe1 = getshiftedDownPipe()
        newPipe2 = getshiftedDownPipe()
    ...
    while True:
        ...
        if 0 < upperPipes[0]['x'] < 5:
            if parameter is 0:
                ...
                newPipe = getshiftedUpPipe()
            elif parameter is 3:
                newPipe = getshiftedDownPipe()
        ...
        # print species ID:
        showSpeciesID(neural_network.species_number, x_position=0.999, y_position=0.04, text="species")
...
def getshiftedUpPipe():
    ...
    return [
        {'x': pipeX, 'y': 100 - pipeHeight},  # upper pipe
        {'x': pipeX, 'y': 200}, # lower pipe
    ]


def getshiftedDownPipe():
    """returns a randomly generated pipe"""
    if (RANDOM_PIPES):
        random.seed()
    else:
    ...
    return [
        {'x': pipeX, 'y': 215 - pipeHeight},  # upper pipe
